...
import { farmBundle } from '@/assets/data/restaurant';
...
const Details = () => {
  ...
  const DATA = farmBundle.items.map((item, index) => ({
    title: item.category,
    data: item.products,
    index,
  }));
  ...
  return (
    <>
      <ParallaxScrollView
        ...
        renderBackground={() => <Image source={farmBundle.img} style={{ height: 300, width: '100%' }} />}
        contentBackgroundColor={Colors.lightGrey}
        renderStickyHeader={() => (
          <View key="sticky-header" style={styles.stickySection}>
            <Text style={styles.stickySectionText}>{farmBundle.name}</Text>
          </View>
        )}>
        <View style={styles.detailsContainer}>
          <Text style={styles.restaurantName}>{farmBundle.name}</Text>
          <Text style={styles.restaurantDescription}>
            {farmBundle.delivery} · {farmBundle.tags.map((tag, index) => `${tag}${index < farmBundle.tags.length - 1 ? ' · ' : ''}`)}
          </Text>
          <Text style={styles.restaurantDescription}>{farmBundle.about}</Text>
          <SectionList
      ...
      <Animated.View style={[styles.stickySegments, animatedStyles]}>
        <View style={styles.segmentsShadow}>
          <ScrollView ref={scrollRef} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.segmentScrollview}>
            {farmBundle.items.map((item, index) => (
              <TouchableOpacity
